//#region node_modules/@rails/activestorage/app/assets/javascripts/activestorage.esm.js
...
var FileChecksum = class FileChecksum {
	static create(file, callback) {
		new FileChecksum(file).create(callback);
	}
	constructor(file) {
		this.file = file;
...
function toArray(value) {
	...
	else if (Array.from) return Array.from(value);
	else return [].slice.call(value);
}
var BlobRecord = class {
	constructor(file, checksum, url, customHeaders = {}) {
		...
		if (csrfToken != void 0) this.xhr.setRequestHeader("X-CSRF-Token", csrfToken);
	...
	requestDidLoad(event) {
		...
		} else this.requestDidError(event);
	}
	...
	toJSON() {
		const result = {};
		for (const key in this.attributes) result[key] = this.attributes[key];
		return result;
	}
};
var BlobUpload = class {
	constructor(blob) {
		...
		for (const key in headers) this.xhr.setRequestHeader(key, headers[key]);
		this.xhr.addEventListener("load", ((event) => this.requestDidLoad(event)));
		this.xhr.addEventListener("error", ((event) => this.requestDidError(event)));
	}
	create(callback) {
	...
	requestDidLoad(event) {
		...
		if (status >= 200 && status < 300) this.callback(null, response);
		else this.requestDidError(event);
	}
	requestDidError(event) {
		...
		FileChecksum.create(this.file, ((error, checksum) => {
			...
			blob.create(((error) => {
				if (error) callback(error);
					...
					upload.create(((error) => {
						if (error) callback(error);
						else callback(null, blob.toJSON());
					}));
				}
			}));
		}));
...
function notify(object, methodName, ...messages) {
	if (object && typeof object[methodName] == "function") return object[methodName](...messages);
}
var DirectUploadController = class {
	...
	start(callback) {
		...
		this.directUpload.create(((error, attributes) => {
			...
			} else hiddenInput.value = attributes.signed_id;
			this.dispatch("end");
			callback(error);
		}));
	}
	uploadRequestDidProgress(event) {
		...
		if (progress) this.dispatch("progress", { progress });
	}
	get url() {
	...
	dispatchError(error) {
		if (!this.dispatch("error", { error }).defaultPrevented) alert(error);
	}
	...
	simulateResponseProgress(xhr) {
		...
		const updateProgress = () => {
			...
			progress = 90 + Math.min(elapsed / estimatedResponseTime, 1) * 9;
			this.dispatch("progress", { progress });
			if (xhr.readyState !== XMLHttpRequest.DONE && progress < 99) requestAnimationFrame(updateProgress);
		};
	...
	estimateResponseTime() {
		...
		const MB = 1024 * 1024;
		if (fileSize < MB) return 1e3;
		else if (fileSize < 10 * MB) return 2e3;
		else return 3e3 + fileSize / MB * 50;
	}
};
...
var DirectUploadsController = class {
	...
	start(callback) {
		...
		const startNextController = () => {
			...
			if (controller) controller.start(((error) => {
				if (error) {
					callback(error);
					this.dispatch("end");
...
function didClick(event) {
	...
	if (button && button.type === "submit" && button.form) submitButtonsByForm.set(button.form, button);
}
...
function handleFormSubmissionEvent(event) {
	...
	if (inputs.length) {
		...
		controller.start(((error) => {
			...
			if (error) inputs.forEach(enable);
