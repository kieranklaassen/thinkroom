...
- An activity-feed row or agent event for a browser-driven replacement. There is no web route to record an activity from the browser, and the snapshot POST is the viewer's session; adding attribution there is a separate decision. The tool result tells the agent no activity was logged.
...
- `Suggestion.auto_reject_stale!` for pending suggestions whose targets vanish; they will fail "missing" on accept as they do after any large human edit.
