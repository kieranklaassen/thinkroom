...
- R8. The panel says when the document text has changed since the last run and shows how many findings are marked changed.
...
- KTD3. **The client supplies the judged text.** The compound panel projects the live ProseMirror document into ordered paragraphs (textblocks, text nodes only, code blocks and table cells skipped, each with `index`, `kind`, `text`) and posts them with the run. The server segments sentences and 1-3 word n-grams that do not cross commas, semicolons, colons, or dashes. A finding stores `paragraph_index`, `paragraph_text`, `quote`, `quote_offset`, and `scope`. The client anchors it in three steps: a textblock with identical text (preferring the stored index among duplicates) maps the stored offset through the same text projection; failing that, the textblock at the stored index (or its immediate neighbours) re-anchors the finding when the quote occurs in it exactly once; otherwise the finding is changed. (Chosen over judging the server's `content_markdown`: the snapshot lags the live document and its flattening cannot map back to editor positions.) Governs R3, R9, R10.
...
- KTD8. **Configuration.** `config/initializers/ruby_llm.rb` sets `typesafe_api_key` from `ENV["TYPESAFE_API_KEY"]`, `request_timeout`, and retries once. `CompoundWriting.enabled?` is true when the key is present or the fake judge is on. `config/deploy.yml` adds `TYPESAFE_API_KEY` to `env.secret` when `KAMAL_COMPOUND_WRITING=1`; `.kamal/secrets.example`, `.kamal/deploy.env.example`, and `DEPLOYING.md` document it. `config/initializers/filter_parameter_logging.rb` already filters params whose name contains `_key`; the paragraphs payload is not filtered. Governs R7, R18, R19.
- KTD9. **Caps and guards.** A pass accepts at most 6,000 words, 60,000 characters, and 400 paragraphs; over the cap the controller returns the existing Inertia error-bag redirect with a message. Runs require write access through `with_document_write_access` and reuse `rate_limit_contributions`. Paragraph-scope questions skip paragraphs under 15 words; headings receive phrase questions only. Governs R16.
...
4. Duplicate paragraphs anchor to the stored index; deleting the first duplicate does not move findings onto the second.
