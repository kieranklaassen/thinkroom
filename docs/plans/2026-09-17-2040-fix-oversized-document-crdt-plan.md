...
- KTD7. **The sync-reply is sent only when the client owes the server something.** `CableProvider` tracks whether any local doc update happened since the last handshake (`pendingLocalChanges`; the page's server-rendered hydration is tagged `server-hydrate` and does not count; an `update` sent on a live connection can still be lost before a disconnect, and a delete-only edit leaves no structs to notice) and inspects the encoded diff's leading varUint (zero clients with structs means delete-set only). It replies when either says so; a client that only hydrated sends nothing. `replaceDocumentContent` (the WebMCP in-page replace) checks the same guard before dispatching and refuses with `too_large` instead of reporting a filtered transaction as persisted. The server keeps its fold-time handling of delete-set echoes for older clients. Governs R9, R9a.
