...
class WritingPass < ApplicationRecord
  ...
  RUN_STATUSES = %w[queued running finished failed].freeze
  MAX_WORDS = 6_000
  MAX_CHARS = 60_000
  MAX_PARAGRAPHS = 400
  ...
  def self.start!(document:, requested_by_name:, reviewer_keys:, paragraphs:)
    keys = CompoundWriting::Reviewers.select(reviewer_keys)
  ...
  def reviewers = reviewer_keys.map { |key| CompoundWriting::Reviewers.find!(key) }
  ...
  def record_run!(reviewer_key, status:, error: nil, findings_count: nil)
    raise ArgumentError, "unknown run status #{status.inspect}" unless RUN_STATUSES.include?(status)
